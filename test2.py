...
def reformat_proxies(proxy_list, usernames, proxy_type="http"):
    formatted_proxies = []
    for i, proxy in enumerate(proxy_list):
        print(f"Raw proxy: {proxy}")  # Debug output to see the raw proxy format
        parts = proxy.split(":")
        if len(parts) >= 2:  # Expecting 'IP:PORT' or 'IP:PORT:username:password'
            ip_port = parts[0] + ":" + parts[1]
            username = usernames[i % len(usernames)]  # Cycle through usernames
            formatted_proxy = f"{proxy_type}://{username}:pratik205@{ip_port}"
...
def main():
    ...
    formatted_proxies = reformat_proxies(all_proxies, usernames, proxy_type="http")  # Specify the proxy type here
